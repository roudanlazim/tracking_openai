...
def apply_proposed_sg_to_csv(mapping_csv_path, scan_groups_path, output_csv_path, proposed_matches, run_label=None):
    """
    ...
    try:
        ...
        col_name = f"proposed_sg_{run_label or timestamp}"
        counter = 1
        ...
        df[col_name] = df["scan_name"].apply(get_proposed)
        ...
        log_entries = []
        for scan_name in df["scan_name"]:
            proposed = df.at[df[df["scan_name"] == scan_name].index[0], col_name]
            log_entries.append({
                "scan_name": scan_name,
