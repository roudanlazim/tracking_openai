...
### **Data Files**
- **`data/scan_groups.json`** → Defines action-required scans and their impact.
- **`data/status_elements.json`** → List of valid final statuses.
- **`data/resolving_scans.json`** → Defines scans that resolve action-based issues (e.g., customs release, arrival at depot).
- **`data/ai_stories.csv`** → Input CSV containing AI stories and expected results.
- **`data/results.csv`** → Output CSV with predicted statuses.
...
- If `"Delivered"` or a variation (e.g., `"Delivered to a safe place"`, `"Driver release"`) exists → **use it as the final status.**
...
1️⃣ **Generate Training & Test Data** (`generate_data.py`)
2️⃣ **Preprocess & Structure Data** (`prepare_data.py`)
3️⃣ **Fine-Tune OpenAI Model** (`fine_tune.py`)
4️⃣ **Predict Shipment Statuses** (`predict.py`)
5️⃣ **Test Model Accuracy & Flag Issues** (`ai_status_testing.py`)

---

## **Next Steps**
- 🚀 **Retrain the model with new structured datasets.**
- 🔎 **Review flagged cases manually for refinement.**
- 📈 **Monitor prediction accuracy and improve resolving scans logic.**
- 🛠 **Optimize API efficiency by reducing token usage in predictions.**
...
def backup_old_jsonl(output_jsonl):
    ...
    if os.path.exists(output_jsonl):
        ...
        logging.info(f"🔄 Previous training JSONL file backed up: {output_jsonl}_backup.jsonl")
...
def prepare_training_data(input_csv, output_jsonl, status_elements_file):
    ...
    try:
        ...
        if df.empty:
            logging.error("❌ Error: Training CSV file is empty.")
        ...
        with open(output_jsonl, "w") as f:
            for _, row in df.iterrows():
                ...
                if correct_status not in valid_statuses:
                    logging.warning(f"⚠️ Skipping row with invalid status: {correct_status}")
                    skipped_count += 1
                    continue
        ...
        print(f"✅ Training data saved to {output_jsonl} ({processed_count} valid entries, {skipped_count} skipped)")
...
Size Recommendation: At least 5-10 variations per status element (~1,500–3,000 examples total).
...
Common transitions (Manifested → Collected → In Transit → Delivered).
...
## **1️⃣ Pre-Processing: AI Story Filtering (Before AI Querying)**
...
- **Auto-Assign Final Statuses**:
  - If *Delivered* is present → **Auto-mark as final status** (No AI Query Required).
  - If *Returned to Sender* is present → **Auto-mark**.
- **Problem-Resolution Handling**:
  - If *Customs Delay* occurs but later *Released by Customs* appears → **Ignore the delay scan** and consider the resolution.
  - If *Request to Reschedule Delivery* is followed by *Out for Delivery* → **Ignore the request**.
- **Ignore Non-Impactful Scans**:
  - *Billing information received* → Ignore unless it's the only scan.
  - *Shipment manifested* → Ignore unless it appears only with other ignorable scans.
  - *Internal scans* (e.g., *Warehouse Scan*, *Hub Arrived*) → Ignore unless affecting movement status.

**Outcome**: The AI model receives a **cleaned and optimized AI Story** with only meaningful status elements.(this is extra - ignore)

---

## **2️⃣ AI Model Training Strategy (Staged Approach)**
...
## **3️⃣ Post-Prediction Validation & Auto-Correction**
After the AI predicts a status, we validate and auto-correct it:

### **Post-Processing Steps:**
- **If AI Prediction is NOT in the learned list** → Use **string similarity matching** to auto-correct.
- **If multiple possible statuses exist** → Choose the highest priority:
  - *Delivered* > *Out for Delivery* > *In Transit*
  - *Released by Customs* > *Customs Delay*
  - *Handover to Broker* > *Pending Customs Clearance*
- **Fallback Handling**:
  - If AI outputs an *unrecognized* status → Flag for review.
  - If AI prediction conflicts with a known rule → Auto-correct based on resolution logic.
